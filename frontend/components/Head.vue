<template>
  ...
  <nav id="header" class="w-full z-30 top-0 py-1 shadow">
    <div
      class="w-full container mx-auto flex flex-wrap items-center justify-between mt-0 px-2 md:px-6 py-3"
    >
      ...
      <div class="order-1 md:order-2">
        ...
        >
          <svg
            class="hidden md:block fill-current text-gray-800 mr-2"
            xmlns="http://www.w3.org/2000/svg"
            width="32"
            height="32"
            viewBox="0 0 24 24"
      ...
      <div id="nav-content" class="order-2 md:order-3 flex items-center">
        ...
        >
          <div class="flex">
            ...
            <span
              class="inline-block bg-teal-500 text-white text-tiny px-2 py-1 rounded-full"
...
export default {
  async fetch() {
    try {
      this.pages = await this.$strapi.$http.$get("/pages/menu");
    } catch (error) {
      this.error = error;
    }
  },
